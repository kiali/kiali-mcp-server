...
func (k *Kiali) AppTraces(ctx context.Context, authHeader string, namespace string, app string, queryParams map[string]string) (string, error) {
...
//   - queryParams: optional query parameters map for filtering traces (e.g., "startMicros", "endMicros", "limit", "minDuration", "tags", "clusterName")
func (k *Kiali) ServiceTraces(ctx context.Context, authHeader string, namespace string, service string, queryParams map[string]string) (string, error) {
	baseURL, err := k.validateAndGetBaseURL()
	if err != nil {
		return "", err
	}
...
//   - queryParams: optional query parameters map for filtering traces (e.g., "startMicros", "endMicros", "limit", "minDuration", "tags", "clusterName")
func (k *Kiali) WorkloadTraces(ctx context.Context, authHeader string, namespace string, workload string, queryParams map[string]string) (string, error) {
	baseURL, err := k.validateAndGetBaseURL()
	if err != nil {
		return "", err
	}
